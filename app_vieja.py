...
def op7_1():
    ...
    if palabras[:1]=="NO HAY": # No se ha encontrado el programa
        teclado.Enter()
        teclado.Enter()
        return render_template("listado.html")
    else: # Se ha encontrado el programa
        programa = read_line(1).split()

        if programa[2]==nombre:
            data = {
                "numero": programa[0],
                "nombre": programa[2],
                "tipo"  : programa[3],
            ...
            return render_template("op7_1.html", data=data)
    
    return redirect('/op')

@app.route('/op7_2', methods=['POST'])
def op7_2():
    return redirect('/op')
